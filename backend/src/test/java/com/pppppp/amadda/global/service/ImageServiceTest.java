...
class ImageServiceTest extends IntegrationTestSupport {
    ...
    void saveKakaoImgInS3() throws MalformedURLException {
        ...
        String newUrl = String.format("https://amadda-bucket.s3.ap-northeast-2.amazonaws.com/%s",
            fileName);

        // when
        given(s3.getUrl(anyString(), anyString()))
            .willReturn(new URL(newUrl));
        given(s3.putObject(anyString(), anyString(), any(InputStream.class),
